...
4. **Suggest scope boundaries proactively.** BRN works best with features completable in ~10-15 `/next` runs. If the user describes something massive, propose a split: "This sounds like 3 features: auth, CRUD, and real-time sync. Want to spec auth first? Each can build on the previous."
...
- [ ] **Scope fits ~10-15 /next runs** — if it's bigger, it should have been split in Step 3.
...
- **Default to small scope.** It's easier to add a follow-up spec than to cut scope from an in-progress feature. When in doubt, suggest "v1" scope and note what "v2" could add.
